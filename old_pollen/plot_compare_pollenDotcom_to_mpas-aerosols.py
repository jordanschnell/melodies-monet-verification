...
for ispec in obs_species_list:
   ...
   if fcst_type == 0:
      ...
      figtitle   = 'pollen.com for ' + today + ' vs. MPAS-Aerosols 1 day-forecast '
   else:
...
#   axes[2*knt].axis('off')
   # Model
   contour2 = axes[2*knt+1].contourf(mdl_lons, mdl_lats, mdl_pollen/10., levels=lvls, norm=norm, cmap=newcmp,transform=ccrs.PlateCarree(),projection=ccrs.PlateCarree())
...
#
   plt_name = 'plot_grp3.pollen_'+ispec+'_'+today+'_PollenDotCom_MPAS-Aerosols_CONUS_'+str(fcst_type) + '.png'
