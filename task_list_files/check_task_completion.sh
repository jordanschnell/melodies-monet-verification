...
do
  ...
  do
    ...
    do
      if [[ -e ${SCRIPTS_DIR}/task_list_files/do_${model_type[$i]}_${species[$j]}_${platform[$j]}_${plottype[$k]}.txt ]]; then
         if [[ ! -e ${LOG_DIR}/mm_run_${START_TIME}_${model_type[$i]}_${species[$j]}_${platform[$j]}_${plottype[$k]}.log ]]; then
            exitstatus=1
            break
         fi
      fi
